...
#                                                                                  #
#                  5. Order of observed data fed into model MUST match the         #
#                     order of the ODE equations.                                  #
#                     NOTE: ODE equations with NO data should be at the end        #
#                           of your ODE model                                      #
####################################################################################
...
def CostFunction(params, ODE_model, init_cond, obs_data, sample_times, unique_times, params_trans=None, ODE_names=None, return_all=False):

    # These lines default transformation to be linear unless specified otherwise by the user
    if params_trans is None:
        params_trans = ["linear"] * len(params)
    ...
    # This line calculates the number of ODE solutions present WITH data
    n_ODE_sol = len(obs_data)
    ...
    for i in range(n_ODE_sol):
        ...
        # EVENTUALLY ADD LOGIC STATEMENTS SO USERS CAN SELECT DIFFERENT ERROR FUNCTIONS
    ...
    if return_all:
        ...
        if ODE_names is not None:
            if len(ODE_names) != n_ODE_sol:
                raise ValueError("The number of ODE equations/names entered in ODE_names needs to match the number of ODE outputs your ODE_model returns (so maybe double-check your list of ODE_names) :)")
...
def boot_run(b, ODE_model, init_cond, obs_data, sample_times, unique_times, bounds, maxiter, tol, rmsle_return, ODE_names, params_trans):

    # These lines create empty lists to store the bootstrap samples in for running in DE
    obs_b = []
    sampleTime_b = []
    uniqueTime_b = []
    ...
    # This line runs the bootstraps through Differential Evolution
    optimized_b = differential_evolution(
        lambda params: CostFunction(params, ODE_model, init_cond,
                                    obs_b, sampleTime_b, uniqueTime_b,
                                    params_trans, ODE_names, return_all=False),
        bounds=bounds,
        maxiter=maxiter,
        tol=tol)

    return optimized_b.x
...
def DE_Generalized(ODE_model, init_cond, obs_data, sample_times, unique_times, bounds, n_boot=0, n_jobs=1, params_trans=None, rmsle_return=False, maxiter=10000, tol=1e-5, plot_cost_hist=True, ODE_names=None, show_usage=False):

    # These lines provide error checks
    ...
    # These lines create a function to store the cost history of each full model (not the bootstrap models)
    # This line creates an empty list to store the cost history
    cost_hist_full = []

    # This line defines the function to pull out the cost history for graphing
    def cost_record_full(xk, convergence):
        full_cost = CostFunction(xk, ODE_model, init_cond,
                                 obs_data, sample_times, unique_times, params_trans,
                                 ODE_names, return_all=False)
        cost_hist_full.append(full_cost)
        return False
    
    # This line runs the full data set through Differential Evolution
    ...
    RMSLE_Results = CostFunction(optParams_full,ODE_model, init_cond,
                                 obs_data, sample_times, unique_times,
                                 params_trans, ODE_names, return_all=rmsle_return)
    ...
    # This line specifies to stop and return results after running the full model if the number of bootstraps is set to 0 in the argument line
    if n_boot == 0:
        ...
        bootstrap_loop_results = Parallel(n_jobs=n_jobs, verbose=10)(
            delayed(boot_run)(b, ODE_model, init_cond, obs_data, sample_times,
                              unique_times, bounds, maxiter, tol, rmsle_return, ODE_names, params_trans)
            for b in range(n_boot))

        # These lines specify the number of parameters that are being estimated and how many bootstrap estimates of each parameter there are (for storing)
        n_params = len(bounds)
        boot_params = np.zeros((n_boot, n_params))

        # This for loop works through the bootstrap results and stores each parameter estimate from each bootstrap in the pre-defined list boot_params
        for b, params_b in enumerate(bootstrap_loop_results):
            boot_params[b, :] = params_b

        # Plotting cost histories for full model
        if plot_cost_hist:
        ...
        return (optParams_full, (Total_RMSLE_Full, RMSLEs_Full), boot_params)
...
def DE_Results(DE_output, ODE_model, init_cond, unique_times, bounds, conf_int=(2.5, 97.5), params_trans=None, param_names=None, show_usage=False):

    # These lines pull out the optimized paramters for both the full model and the bootstraps
    optParams_full = DE_output[0]

    # These lines default transformations to be linear unless specified otherwise by the user
    if params_trans is None:
        params_trans = ["linear"] * len(optParams_full)

    # This line creates an empty list to store un-transformed/original parameters
    transformed_params_full = []
    
    # This for loop works through parameters specified by user to undo log_10 transformations or leave parameters unchanged (as specified by the user).
    # Specification for transformations, or lack thereof, should be entered as a list: params_trans = ['linear', 'log10', 'log10', 'linear', etc.]
    # NOTE: Parameter transformation specifications must match the exact order parameters are specified in. So, if you have parameters a, b, c, and d,
    #       you enter the parameters as params = ['a', 'b', 'c', 'd'], and you log-transformed b and c, then you would need to specify the
    #       order for params_trans as: params_trans = ['linear', 'log10', 'log10', 'linear'] to un-log transform b and c for the ODE solver.
    ...
    # This line puts the full model parameters into a usable form for computing fitted lines
    ...
    # These lines create the data for the confidence interval of the ODE system from the bootstraps
    n_ODE_sol = ODE_full.shape[1]

    # This line performs a check to see if bootstrapping was done in DE_Generalized
    if not len(DE_output) > 2:

    # These lines give parameters an index name if no list of specific names was provided
        if param_names is None:
            param_names = [f"param_{i+1}" for i in range(len(params_trans))]

        # This line creates the data frame of the parameter estimates without confidence intervals when n_boot=0
        param_est_df = pd.DataFrame({"Parameter": param_names,
                                     "Estimate (Full Model)": transformed_params_full,
                                     "Lower CI": [None] * len(optParams_full),
                                     "Upper CI": [None] * len(optParams_full),
                                     "Parameter Bounds": bounds})
        
        # This line returns a dictionary without confidence interval estimates for each parameter when n_boot=0
        return {"time": t_list,
                "Full ODE Solutions": ODE_full,
                "Bootstrap ODE Solutions": None,
                "Lower CI: Fit": None,
                "Upper CI: Fit": None,
                "Full ODE Parameter Estimates": transformed_params_full,
                "Bootstrap ODE Parameter Estimates": None,
                "Lower CI: Parameters": None,
                "Upper CI: Parameters": None,
                "Parameter Table": param_est_df}

    # These lines pull out the bootstrapping results when n_boot > 0
    optParams_boot = np.asarray(DE_output[2])
    n_boot, n_params = optParams_boot.shape

    # This line creates an empty list to store parameters in (whether left as entered, or untransformed)
    transformed_params_boot = []

    # These lines loop through the parameters in the bootstrap results and untransform parameters from log scale is specified by the user earlier
    for b in range(n_boot):
        boot_param_vec = []
    ...
    # This line puts the bootstrapped model parameters into a usable form for computing fitted lines and confidence intervals
    transformed_params_boot = np.asarray(transformed_params_boot)

    # This line creates an empty list to store the fitted curves from the bootstrapping
    boot_fit = np.zeros((n_boot, len(t_list), n_ODE_sol))

    # These lines compute the ODE solutions using the bootstrapped samples
    ...
    # These lines compute the upper and lower bounds of the confidence interval for the fitted curve
    CI_lower = np.percentile(boot_fit, conf_int[0], axis=0)
    CI_upper = np.percentile(boot_fit, conf_int[1], axis=0)

    # These lines compute the upper and lower bounds of the confidence interval for each parameter
    ...
    # This line creates a dataframe with all of the results when n_boot > 0
    param_est_df = pd.DataFrame({"Parameter": param_names,
                                 "Estimate (Full Model)": transformed_params_full,
    ...
    # This returns a dictionary with the results when n_boot > 0
    return {"time": t_list,
            "Full ODE Solutions": ODE_full,
...
########################################
# Defining likelihood profile function #
########################################

def DE_profile_likelihood(ODE_model, init_cond, obs_data, sample_times, unique_times, bounds, n_points=100, params_trans=None, param_names=None, maxiter=10000, tol=1e-5):

    # This line calculates the number of parameters to calculate the profile likelihoods for
    n_params = len(bounds)
    
    # These lines default transformation to be linear unless specified otherwise by the user
    if params_trans is None:
        params_trans = ["linear"] * n_params

    # These lines default the names of parameters to be numerical if a list of parameter names where not provided
    if param_names is None:
        param_names = [f"param_{i+1}" for i in range(n_params)]

    # This line creates an empty dataframe to store profile likelihood results
    pl_summary = pd.DataFrame({'Parameter': [],
                              'Value': [],
                              'Error': []})

    # These lines create the for loop to loop through each parameter, hold it as fixed, and get the profile likelihoods
    for i in range(n_params):

        # These lines create a grid of parameter values to feed into DE while one parameter remains fixed
        param_min, param_max = bounds[i]
        param_name = param_names[i]
        param_vals = np.linspace(param_min, param_max, n_points)

        # This line pulls out the parameter bounds from the original bounds provided in the argument line
        new_bounds = [bound for bound in bounds]

        # This line creates an empty list to store the errors
        errors = []

        # These lines run DE to get the profile likelihoods
        for p_val in param_vals:

            # This line fixes the parameter that the profile likelihood is being obtained for
            new_bounds[i] = p_val, p_val

            # This line calculates the solution to the ODE system with the one fixed parameter and remaining free parameters
            optimized_pl = differential_evolution(
                lambda params: CostFunction(params, ODE_model, init_cond,
                               obs_data, sample_times, unique_times, params_trans,
                               return_all=False),
                bounds=new_bounds,
            ...
            # This adds the error from the above DE run to the list for graphing later
            errors.append(optimized_pl.fun)

        # This line stores the parameter names, values, and associated errors in a dataframe for plotting the profile likelihoods
        pl_summary = pd.concat([pl_summary, pd.DataFrame({'Parameter': param_name,
                                                          'Value': param_vals,
